...
import com.example.ui.screens.OnboardingGuideScreen
...
fun FizzApp(viewModel: TorPeerViewModel = viewModel()) {
  ...
  val hasAcceptedWaiver by viewModel.hasAcceptedWaiver.collectAsStateWithLifecycle()
  val hasSeenOnboardingGuide by viewModel.hasSeenOnboardingGuide.collectAsStateWithLifecycle()
  ...
  BubbleWaterBackground {
    when {
      !hasAcceptedWaiver -> {
        LiabilityWaiverScreen(
          onAcceptWaiver = { viewModel.acceptLiabilityWaiver() }
        )
      }
      !hasSeenOnboardingGuide -> {
        OnboardingGuideScreen(
          onFinishGuide = { viewModel.completeOnboardingGuide() }
        )
      }
      else -> {
        Scaffold(
          ...
          bottomBar = {
            ...
            if (selectedPeerId == null) {
              NavigationBar(
                containerColor = theme.surfaceElevated.copy(alpha = 0.96f),
                tonalElevation = 0.dp,
                modifier = Modifier
                  .border(1.dp, theme.borderGold)
          ...
          Column(
            ...
            // Transparent live activity pill bar across the app
            if (selectedPeerId == null) {
              LiveTransparencyBar(
